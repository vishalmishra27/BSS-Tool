...
RCM_REQUIRED_COLUMNS = [
    "Process", "SubProcess", "Risk Id", "Risk Title",
...
agent_state = {
    "rcm_df": None,
    "original_rcm_df": None,      # original RCM with all original column names (for output)
    "output_dir": None,
...
def exec_load_rcm(args):
    ...
    # Smart reader: header detection + markers + column normalisation (exact + LLM)
    ...
    try:
        ...
        print(f"  [load_rcm] Header detected at row {read_info['header_row']} (score={read_info['score']})")
        if read_info.get("column_map"):
            print(f"  [load_rcm] Mapped {len(read_info['column_map'])} columns")
        if read_info.get("missing"):
            print(f"  [load_rcm] Missing required columns: {read_info['missing']}")
    except Exception as e:
        print(f"  [load_rcm] Smart reader failed ({e}), falling back to plain read + normalize")
        if file_path.endswith(".csv"):
            ...
            if original_df is None:
                original_df = df.copy()
        # Fallback to simple normalisation
    ...
    agent_state["original_rcm_df"] = original_df
...
def exec_test_of_design(args):
    ...
    print(f"  [tod] Evidence loaded for {len(tod_bank)} controls")

    if not tod_bank:
        return {"error": "No evidence files found in the folder. Expected subfolders named by Control ID (e.g. C-P2P-001/)."}

    # Create tester — pass pre-normalised DataFrame to skip redundant file I/O,
    # plus original_rcm_df for the output summary sheet.
    tester = TOD_Engine.RCMControlTester(
        ...
        original_rcm_df=agent_state.get("original_rcm_df"),
    )

    print(f"  [tod] Running Test of Design... (this may take several minutes)")
    results, schemas = tester.test_all_tod(tod_bank, max_workers=5)
    agent_state["tod_results"] = results
    agent_state["tod_schemas"] = schemas
...
def exec_test_of_effectiveness(args):
    ...
    if agent_state.get("tod_results") is not None:
        include_control_ids = {
            r.control_id for r in agent_state.get("tod_results", [])
            if getattr(r, "result", None) == "PASS"
        }
        print(f"  [toe] Parsing evidence only for TOD PASS controls: {len(include_control_ids)}")
    ...
    # Create tester — pass pre-normalised DataFrame (mapping already done in
    # load_rcm / TOD step), plus original_rcm_df for the output summary sheet.
    ...
    results = tester.test_all_toe(
        toe_bank, max_workers=5,
        pre_schemas=agent_state.get("tod_schemas"),
        tod_results=agent_state.get("tod_results"),
    )
