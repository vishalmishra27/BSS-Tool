...
# Azure OpenAI config for TOD/TOE (with OPENAI_* fallback)
AZURE_OPENAI_ENDPOINT = os.getenv("AZURE_OPENAI_ENDPOINT", "https://entgptaiuat.openai.azure.com").strip()
AZURE_OPENAI_API_KEY = os.getenv("AZURE_OPENAI_API_KEY", "808cf0ccab8445b39c6d8767a7e2c433").strip()
AZURE_OPENAI_API_VERSION = os.getenv("AZURE_OPENAI_API_VERSION", "2023-07-01-preview").strip()
AZURE_OPENAI_DEPLOYMENT_NAME = os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME", "gpt-4o-mini").strip()

OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", AZURE_OPENAI_API_KEY).strip()
OPENAI_MODEL = os.getenv("OPENAI_MODEL", AZURE_OPENAI_DEPLOYMENT_NAME).strip()
TOD_MAX_WORKERS = 5
TOE_MAX_WORKERS = 5
...
def tod_test_endpoint():
    """Test of Design (TOD) runner for rcm_tester.py"""
    ...
    try:
        ...
        output_report = str(output_dir / f"TOD_Report_{timestamp}.txt")

        print(f"\n[API] Starting TOD Test...")
        ...
        results = tester.test_all(sample_bank, max_workers=max_workers)
        tester.export_results(results, output_xlsx, sample_bank=sample_bank)
...
def toe_test_endpoint():
    ...
    try:
        ...
        results = tester.test_all_toe(toe_bank, max_workers=TOE_MAX_WORKERS)
