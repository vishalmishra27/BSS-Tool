"""
RCM Control Testing -- Test of Design (TOD) + Test of Operating Effectiveness (TOE)
=====================================================================================
TOD: 1 sample per control. Evaluates whether controls are DESIGNED adequately.
TOE: Multiple samples per control. Evaluates whether controls actually OPERATED correctly.

Both share the same schema generation, evaluation pipeline, and self-correction loop.
TOD returns schemas so they can be cached and reused by TOE (avoiding redundant LLM calls).

Usage (TOD):
    tester = RCMControlTester(...)
    tod_bank = load_tod_evidence_folder("evidence_tod/")
    results, schemas = tester.test_all_tod(tod_bank)
    tester.export_tod_workpaper(results, "tod_workpaper.xlsx")

Usage (TOE):
    tester = RCMControlTester(...)
    toe_bank = load_toe_evidence_folder("evidence_toe/")
    results = tester.test_all_toe(toe_bank, pre_schemas=schemas)  # reuse TOD schemas
...
# ===========================================================================
#  EXCEL SANITISATION — strip chars illegal in XML/openpyxl
# ===========================================================================
...
def _sanitize_for_excel(obj):
    ...
    Works on strings, lists, dicts, and dataclass instances.
    Mutates dataclass objects in-place and returns the same reference.
    """
    if isinstance(obj, str):
        return _ILLEGAL_XML_CHARS_RE.sub("", obj)
...
@dataclass
class TODControlResult:
    """TOD result for one control -- single sample, design adequacy assessment."""
    control_id: str
    risk_id: str
    control_type: str
    nature_of_control: str
    ...
    result: str                    # PASS / FAIL
    design_adequate: str           # "Yes" / "No"
    confidence: str                # "High" / "Medium" / "Low"

    # Deficiency
    deficiency_type: str           # "None" / "Design Deficiency" / "Significant Deficiency" / "Material Weakness"
    gap_identified: str            # description of design gap, or "None"
    overall_remarks: str

    # The single sample result
    sample_result: object = None   # TOESampleResult (reuses same structure)
...
Good example for "CFO approval for recurring expenses" (simple control — 2 attributes is correct):
  1: "CFO Approval" -- Recording expenses are approved by the CFO as per defined authority limits
  2: "Payment After Approval" -- Payment is processed only after the required approval is obtained
...
Bad example — inventing requirements not in control description:
  ...
  These add scope beyond what the control actually states.
...
def _prepare_evidence_for_prompt(rcm: RCMRow, sample: SampleEvidence) -> str:
    ...
    if not use_embed:
        _write_embedding_fallback_log("TOD", rcm.control_id, "EVIDENCE_EMBEDDINGS disabled", len(text), text)
        return text

    try:
        chunks = _split_chunks(text)
        if len(chunks) <= 1:
            _write_embedding_fallback_log("TOD", rcm.control_id, f"Text fits in 1 chunk ({len(text)} chars) — no splitting needed", len(text), text)
            return text
        ...
        if len(vecs) != len(chunks) + 1:
            _write_embedding_fallback_log("TOD", rcm.control_id, f"Embedding API returned wrong vector count (expected {len(chunks)+1}, got {len(vecs)})", len(text), text)
            return text
        ...
        _write_embedding_log(
            engine="TOD", control_id=rcm.control_id, query=query,
        ...
        _write_embedding_fallback_log("TOD", rcm.control_id, f"Exception during embedding: {exc}", len(text), text)
        return text
...
{"- **Control Objective**: " + rcm.control_objective + chr(10) if rcm.control_objective else ""}- **Risk ID**: {rcm.risk_id} | **Risk Title**: {rcm.risk_title}
...
def _enforce_schema_output_quality(eval_result: dict, schema: ControlSchema | None, source_document: str = "") -> dict:
    ...
    missing_attr_ids = []
    if schema and schema.attributes:
        # Build helper map by attribute name (case-insensitive)
    ...
    # Normalize sample_details to prevent blank cells
    ...
    # Guardrail: all attributes N/A should not auto-pass in weak evidence situations
    all_na = False
...
def aggregate_tod_result(
    control_id: str,
    rcm: RCMRow,
    sample_result,    # TOESampleResult (single sample)
) -> TODControlResult:
    """
    Aggregate a single TOD sample into a control-level design adequacy conclusion.

    TOD uses 1 sample per control. The logic is simpler than TOE:
      - PASS -> Design Adequate, no deficiency
      - FAIL -> Design Deficiency (or worse based on control type)
    """
    # Ensure result integrity
    actual_fail = is_sample_fail(sample_result)
    if actual_fail and sample_result.result != "FAIL":
        sample_result.result = "FAIL"
        sample_result.operated_effectively = "No"
        if not sample_result.remarks:
            sample_result.remarks = ""
        sample_result.remarks += " [AUTO-CORRECTED at TOD aggregation]"

    is_fail = sample_result.result == "FAIL"
    ...
    if not is_fail:
        ...
        gap = "None"
        remarks = (
            f"Control design is adequate. The walkthrough sample confirms the control "
            f"is designed to effectively mitigate the identified risk."
        )
    else:
        design_adequate = "No"
        confidence = "High"
        gap = sample_result.deviation_details or sample_result.remarks or "Design gap identified"

        # For automated controls, any design failure is more severe
        if rcm.control_type.strip().lower() == "automated":
            deficiency = "Significant Deficiency"
            remarks = (
                f"Control design is NOT adequate. {gap}. "
                f"As an automated control, this design gap may affect all transactions "
                f"processed by the system. Recommend immediate remediation."
            )
        else:
            deficiency = "Design Deficiency"
            remarks = (
                f"Control design is NOT adequate. {gap}. "
                f"Recommend redesigning the control to address the identified gap."
            )

    return TODControlResult(
        control_id=control_id,
        risk_id=rcm.risk_id,
        control_type=rcm.control_type,
        nature_of_control=rcm.nature_of_control,
...
COLUMN_MAP = {
    ...
    "Risk Probability": "risk_probability", "risk_probability": "risk_probability",
    "Risk Impact": "risk_impact", "risk_impact": "risk_impact",
...
# Default severity ranking for deriving risk_level from probability/impact.
# Higher number = higher severity. The max of the two is chosen.
RISK_SEVERITY_RANK = {
    "low": 1,
    "medium": 2,
    "moderate": 2,
...
RISK_RANK_TO_LABEL = {1: "Low", 2: "Medium", 3: "High", 4: "Critical"}


def derive_risk_level(probability: str, impact: str,
                      severity_rank: dict = None,
                      rank_to_label: dict = None) -> str:
    """Derive risk_level by picking the higher of probability and impact.

    Uses *severity_rank* for ordering and *rank_to_label* for the output label.
    Defaults to RISK_SEVERITY_RANK / RISK_RANK_TO_LABEL when not supplied.
    """
    sr = severity_rank or RISK_SEVERITY_RANK
    r2l = rank_to_label or RISK_RANK_TO_LABEL
    p_rank = sr.get(probability.strip().lower(), 0)
    i_rank = sr.get(impact.strip().lower(), 0)
    chosen = max(p_rank, i_rank)
    return r2l.get(chosen, "High")  # default "High" if not found
...
from Document_Intelligence import write_extraction_summary as _write_extraction_summary
...
def load_toe_evidence_folder(folder_path: str, include_control_ids: set[str] | None = None,
    ...
    # Write parsing summary (combines fresh extractions + pre-cache)
    toe_combined = {_norm_key(fp): extract_cache[fp] for fp in extract_cache}
    toe_combined.update(_pre_cache)
    _write_extraction_summary(toe_combined, phase="TOE")
...
def load_tod_evidence_folder(folder_path: str, pre_extract_cache: dict | None = None,
                             include_control_ids: set[str] | None = None) -> dict:
    """
    Load TOD evidence -- 1 sample per control (design walkthrough).

    Folder structure:
      evidence_tod/
        C-P2P-001/          <- control folder
          walkthrough.txt    <- all files merged into a single evidence block
          SOP.pdf
        C-P2P-002/
          narrative.docx

    All files in each control folder are combined into one SampleEvidence.

    Args:
        folder_path: Root TOD evidence path
        pre_extract_cache: Optional dict[abs_file_path_str -> (content, doc_type, ok)]
            from evidence validation (Phase 2). When provided, files that were
            already extracted via Document Intelligence will be read from cache
            instead of re-extracting, saving significant processing time.
        include_control_ids: Optional set of control IDs to process. Folders not
            matching any ID (case-insensitive) are skipped. If None, all folders
            are processed.

    Returns dict[control_id -> list[SampleEvidence]] (list always has 1 element)
    """
    from concurrent.futures import ThreadPoolExecutor, as_completed

    _set_parse_phase("TOD")

    folder = Path(folder_path)
    if not folder.exists():
        raise FileNotFoundError(f"TOD evidence folder not found: {folder_path}")

    tod_bank: dict[str, list] = {}
    errors: list[str] = []
    format_stats: dict[str, int] = {}

    # Pre-built extraction cache from evidence validation (Phase 2)
    # Keys are absolute path strings → (content_text, doc_type, ok)
    _pre_cache: dict[str, tuple[str, str, bool]] = pre_extract_cache or {}
    cache_hits = 0

    control_dirs = sorted(d for d in folder.iterdir() if d.is_dir() and not d.name.startswith("_"))

    # Filter to only control folders that match RCM control IDs
    if include_control_ids:
        _upper_ids = {cid.strip().upper() for cid in include_control_ids}
        _before = len(control_dirs)
        control_dirs = [d for d in control_dirs if d.name.strip().upper() in _upper_ids]
        _skipped = _before - len(control_dirs)
        if _skipped:
            print(f"[TOD Evidence] Skipped {_skipped} folders not in RCM (processing {len(control_dirs)} controls)")

    if not control_dirs:
        raise FileNotFoundError(
            f"No matching subfolders found in {folder_path}. "
            f"Expected one folder per control (e.g. C-P2P-001/)."
        )

    # ── Phase 1: Collect all evidence files across all controls ──
    # Map: control_id -> list of evidence file paths (preserving order)
    control_files_map: dict[str, list[Path]] = {}
    all_extract_files: list[Path] = []  # non-text files needing DI extraction
    ...
    for control_dir in control_dirs:
        ...
        evidence_files = [
            ...
            if f.name.lower() not in ("readme.txt", "readme.md")
        ]
        ...
        control_files_map[control_id] = evidence_files
        for ef in evidence_files:
            if ef.suffix.lower() not in _TEXT_EXTS:
                # Only extract if NOT already in the pre-built cache
                # Use normalized key for consistency with evidence_validator
                abs_key = os.path.normcase(os.path.normpath(str(ef.resolve())))
                if abs_key in _pre_cache:
                    cache_hits += 1
                else:
                    all_extract_files.append(ef)

    if _pre_cache:
        print(f"[TOD Evidence] Reusing {cache_hits} cached extractions from evidence validation")

    # ── Phase 2: Parallel extraction of ONLY uncached non-text files ──
    parse_workers = int(os.getenv("EVIDENCE_PARSE_WORKERS", "8"))
    extract_cache: dict[Path, tuple[str, str, bool]] = {}
    if all_extract_files:
        print(f"[TOD Evidence] Extracting {len(all_extract_files)} new files via Document Intelligence")
        with ThreadPoolExecutor(max_workers=parse_workers) as executor:
            future_to_path = {executor.submit(_extract_file, fp): fp for fp in all_extract_files}
            for future in as_completed(future_to_path):
                fp = future_to_path[future]
        ...
        print(f"[TOD Evidence] All files served from cache — no Document Intelligence calls needed")

    # Write parsing summary (combines fresh extractions + pre-cache)
    combined_cache = {str(fp.resolve()): extract_cache[fp] for fp in extract_cache}
    combined_cache.update(_pre_cache)
    _write_extraction_summary(combined_cache, control_files_map, phase="TOD")

    # ── Phase 3: Assemble evidence blocks (sequential, uses cached results) ──
    for control_id, evidence_files in control_files_map.items():
        evidence_blocks = []
        for ef in evidence_files:
            ext = ef.suffix.lower()
            format_stats[ext] = format_stats.get(ext, 0) + 1
            ...
            if ext in _TEXT_EXTS:
                # Check pre-cache first for text files too
                abs_key = os.path.normcase(os.path.normpath(str(ef.resolve())))
                if abs_key in _pre_cache:
                    txt, _, ok = _pre_cache[abs_key]
                else:
                    try:
                        ...
                        txt = f"[Read error for {ef.stem}: {e}]"
                        ok = False
            else:
                # Check pre-cache first, then local extract_cache
                abs_key = os.path.normcase(os.path.normpath(str(ef.resolve())))
                if abs_key in _pre_cache:
                    txt, _, ok = _pre_cache[abs_key]
                else:
            ...
            if not ok:
                errors.append(f"  [!]  {control_id}/{ef.stem}: extraction may be incomplete")

        evidence_text = "\n\n".join(evidence_blocks)
        tod_bank[control_id] = [SampleEvidence(
            sample_id=control_id,
            description=evidence_text,
            source_document=", ".join(f.name for f in evidence_files),
            supporting_docs=[],
        )]
    ...
    print(f"[TOD Evidence Loaded] {len(tod_bank)} controls from {folder_path}")
    print(f"[Evidence Files]      {total_files} files across {len(tod_bank)} controls")
    if format_stats:
        label_counts: dict[str, int] = {}
        for ext, n in format_stats.items():
            label = _EXT_LABELS.get(ext, ext.lstrip("."))
    ...
    for cid in sorted(tod_bank):
        n_files = len([f for f in (folder / cid).iterdir() if f.is_file()])
        print(f"  [DIR] {cid}: {n_files} evidence file{'s' if n_files != 1 else ''}")
...
class RCMControlTester:
    ...
    def __init__(self, rcm_path=None, openai_api_key=None, openai_model="gpt-5.2-chat",
        ...
        """
        Args:
            rcm_path:          Path to RCM file. Required when starting fresh (direct TOD/TOE).
        ...
        print(f"[Mode] TOD (1 sample) + TOE (multiple samples) engine\n")
    ...
    # ===================================================================
    #  TOD METHODS (Test of Design -- 1 sample per control)
    # ===================================================================

    def test_all_tod(self, tod_bank: dict, max_workers: int = 5, pre_schemas: dict = None, progress_callback=None) -> tuple:
        """
        Run TOD for all controls -- 1 sample per control.

        Uses the SAME schema generation and evaluation pipeline as TOE.
        The only difference: TOD evaluates exactly 1 walkthrough sample per control.

        Args:
            tod_bank: dict[control_id -> list[SampleEvidence]] from load_tod_evidence_folder()
            max_workers: parallel API workers (default 5)
            pre_schemas: optional dict[control_id -> ControlSchema] from preview step

        Returns:
            (list[TODControlResult], dict[control_id -> ControlSchema])
            The schemas dict is returned so it can be cached for reuse in TOE.
        """
        from concurrent.futures import ThreadPoolExecutor, as_completed
        ...
        for cid, samples in tod_bank.items():
            ...
            if cid not in self.rcm_lookup:
                ...
                continue
            # TOD: take only the first sample
            valid[cid] = [samples[0]] if samples else []

        total_controls = len(valid)

        # Notify caller of total count
        if progress_callback:
            progress_callback("init", 0, total_controls, None)

        # -- Phase 1: Use pre-approved schemas or generate new ones --
        schemas = {}  # control_id -> ControlSchema
        schema_start = time.time()

        if pre_schemas:
            # Use pre-approved schemas from preview step
            schemas_needed = []
            for cid in sorted(valid):
                if cid in pre_schemas:
                    schemas[cid] = pre_schemas[cid]
                else:
                    schemas_needed.append(cid)

            print("=" * 70)
            print(f"  TEST OF DESIGN (TOD) -- 1 sample per control")
            print(f"  Controls to evaluate: {total_controls}")
            print(f"  Pre-approved schemas: {len(schemas)} | Need generation: {len(schemas_needed)}")
            print(f"  Parallel workers: {max_workers}")
            print("=" * 70)
            print()

            if schemas_needed:
                print(f"  Phase 1: Generating {len(schemas_needed)} missing schemas...")
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    future_to_cid = {}
                    for cid in schemas_needed:
                    ...
                    for future in as_completed(future_to_cid):
                        ...
                        try:
                            schemas[cid] = future.result()
                        except Exception as e:
                            print(f"    [!] {cid}: schema failed ({e}) -- using defaults")
                            schemas[cid] = ControlSchema(control_id=cid, worksteps=[], attributes=[], sample_columns=[])
            else:
                print(f"  Phase 1: All {len(schemas)} schemas pre-approved (skipping generation)")
        else:
            total_api_calls = total_controls * 2  # 1 schema + 1 evaluation per control

            print("=" * 70)
            print(f"  TEST OF DESIGN (TOD) -- 1 sample per control")
            print(f"  Controls to evaluate: {total_controls}")
            print(f"  Total API calls: {total_api_calls} ({total_controls} schemas + {total_controls} samples)")
            print(f"  Parallel workers: {max_workers}")
            print("=" * 70)
            print()
        ...
        # -- Phase 2: Evaluate 1 sample per control in parallel --
        print(f"  Phase 2: Evaluating {total_controls} walkthrough samples in parallel...")
        completed = 0
        eval_start = time.time()
        sample_results = {}  # control_id -> TOESampleResult
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for cid in sorted(valid):
                ...
                future = executor.submit(
                    self._evaluate_toe_sample, cid, sample, 1, 1,
                    schema=schemas.get(cid)
                )
                future_map[future] = (cid, sample.sample_id)
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    sample_results[cid] = result
                    icon = "[OK]" if not is_sample_fail(result) else "[FAIL]"
                    re_tag = " [RE-EVAL]" if result.validator_corrected else ""
                    print(f"  [{completed:>3}/{total_controls}] {icon} {cid} | {'PASS' if not is_sample_fail(result) else 'FAIL'}{re_tag}")
                    if progress_callback:
                        progress_callback("eval", completed, total_controls, {
                            "control_id": cid, "result": "PASS" if not is_sample_fail(result) else "FAIL",
                        })
                except Exception as e:
                    print(f"  [{completed:>3}/{total_controls}] [!]  {cid} | ERROR: {e}")
                    ...
                    sample_results[cid] = error_result
                    if progress_callback:
                        progress_callback("eval", completed, total_controls, {
                            "control_id": cid, "result": "ERROR",
                        })

        eval_elapsed = time.time() - eval_start
        total_elapsed = time.time() - schema_start

        re_eval_count = sum(1 for sr in sample_results.values() if sr.validator_corrected)
        total_api_actual = total_controls + len(sample_results) + re_eval_count
        print(f"\n  Phase 2 complete: {len(sample_results)} samples in {eval_elapsed:.1f}s")
        if re_eval_count > 0:
            print(f"  Re-evaluations: {re_eval_count} samples sent back to LLM for self-correction")
        print(f"  Total elapsed: {total_elapsed:.1f}s ({total_api_actual} API calls, {max_workers} workers)")
        print()
        ...
        for cid in sorted(valid):
            rcm = self.rcm_lookup[cid]
            sr = sample_results.get(cid)
            if not sr:
                continue
            tod_result = aggregate_tod_result(cid, rcm, sr)
            tod_result.schema = schemas.get(cid)
            control_results.append(tod_result)

        # -- Append placeholder results for RCM controls with NO evidence folder --
        evaluated_cids = set(r.control_id for r in control_results)
        no_evidence_cids = sorted(cid for cid in self.rcm_lookup if cid not in evaluated_cids)
        for cid in no_evidence_cids:
            rcm = self.rcm_lookup[cid]
            placeholder = TODControlResult(
                control_id=cid,
                risk_id=rcm.risk_id,
                control_type=rcm.control_type,
                nature_of_control=rcm.nature_of_control,
                control_frequency=rcm.control_frequency,
                result="NO EVIDENCE",
                design_adequate="Unable to Assess",
                confidence="N/A",
                deficiency_type="Unable to Assess",
                gap_identified="Evidence folder not found for this control.",
                overall_remarks="Evidence folder not found — no TOD evidence was provided for this control. Manual review required.",
                sample_result=None,
                evaluation_timestamp=time.strftime("%Y-%m-%d %H:%M:%S"),
                schema=None,
            )
            control_results.append(placeholder)
            print(f"  [SKIP] {cid}: Evidence folder not found — added placeholder result")
        if no_evidence_cids:
            print(f"  {len(no_evidence_cids)} control(s) had no evidence folder — placeholder results added")

        passed = sum(1 for r in control_results if r.result == "PASS")
        failed = sum(1 for r in control_results if r.result == "FAIL")
        no_evidence = sum(1 for r in control_results if r.result == "NO EVIDENCE")

        print(f"{'='*70}")
        print(f"  TOD COMPLETE in {total_elapsed:.1f}s")
        print(f"  Controls: {len(control_results)} (RCM universe)")
        print(f"  [OK] Design Adequate: {passed} | [FAIL] Design Gap: {failed} | [SKIP] No Evidence: {no_evidence}")
        print(f"{'='*70}")

        return control_results, schemas

    def export_tod_workpaper(self, results: list, output_path: str,
                             tod_bank: dict = None, company_name: str = "",
                             prepared_by: str = "", reviewed_by: str = ""):
        """
        Export TOD results using the SAME format as TOE workpaper.

        Converts TODControlResult -> TOEControlResult internally, then
        delegates to export_toe_workpaper() so the output is identical.
        """
        # Convert TODControlResult -> TOEControlResult for the TOE exporter
        toe_results = []
        for r in results:
            sr = r.sample_result
            sample_results = [sr] if sr else []

            if r.result == "NO EVIDENCE":
                # Placeholder for controls with no evidence folder
        ...
        self.export_toe_workpaper(
            toe_results, output_path,
            toe_bank=tod_bank,
    ...
    def test_all_toe(self, toe_bank: dict, max_workers: int = 5, pre_schemas: dict = None, tod_results: list = None) -> list:
        """
        ...
        Args:
            ...
            max_workers: parallel API workers (default 5)
            tod_results: optional list[TODControlResult] from TOD run. Controls with
                         result == 'FAIL' or 'NO EVIDENCE' will be skipped in TOE and
                         given placeholder results instead.
    ...
    def export_toe_workpaper(self, results: list, output_path: str,
        ...
        # Sanitize all result objects to strip illegal XML characters that
        # openpyxl rejects (control chars from LLM-generated text / evidence).
        for r in results:
        ...
        try:
            ...
            coso_map = {}
        n_assert = N_ASSERTION_COLS   # 6
        n_ifc = N_IFC_COLS            # 5
        n_coso = N_COSO_COLS          # 8
        n_classification = n_assert + n_ifc + n_coso  # 19 total
        ...
        fill_group_rcm = PatternFill(start_color="00338D", end_color="00338D", fill_type="solid")       # dark navy
        fill_group_assert = PatternFill(start_color="005EB8", end_color="005EB8", fill_type="solid")    # blue
        fill_group_ifc = PatternFill(start_color="483698", end_color="483698", fill_type="solid")       # purple
        fill_group_coso = PatternFill(start_color="009A44", end_color="009A44", fill_type="solid")      # dark green
        fill_group_result = PatternFill(start_color="470A68", end_color="470A68", fill_type="solid")    # dark brown
        ...
        # Financial Assertions headers (blue)
        ...
        deficiency_col = result_start_col + 6  # 7th result column = Deficiency Type

        # -- Write data rows (start from row 3 now due to group header in row 1) --
        ...
        result_widths = [45, 14, 14, 14, 12, 14, 22, 60, 60, 55]
