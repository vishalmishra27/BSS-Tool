...
const ContractModal = ({ isOpen, onClose, onProceedToAnalysis }) => {
  const [uploadedFiles, setUploadedFiles] = useState([]);

  const handleFileUpload = (files) => {
    const pdfFiles = Array.from(files).filter(file => file.type === 'application/pdf');
    const newFiles = pdfFiles.map(file => ({
      id: Date.now() + Math.random(), name: file.name, size: file.size, file: file
    }));
  ...
  return ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Contract
Analysis</h2>
            <p className="text-gray-600">Upload contract PDF files for
analysis</p>
          </div>
        ...
        <div
          className="border-2 border-dashed rounded-lg p-8 text-center mb-6 transition-colors hover:border-blue-400"
        >
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {uploadedFiles.map((file) => ( <div key={file.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-800">{file.name
}</p>
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            <CheckCircle size={16} />
            <span>Proceed to PDF Analysis</span>
          </button>
        </div>
      </div>
    </div>
